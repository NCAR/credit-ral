...
def zscore_var(conf, varname, ind_level=None, flag_float64=True, skipna=False):
    ...
    '''
    # ------------------------------------------------------------------------------------ #
    # lists files and open as xr.Dataset
    filenames = sorted(glob(conf['zscore'][varname]))
    file_nums = int(conf['zscore']['file_nums'])
    train_files = filenames[:file_nums]
    ...
    for i_fn, ds in enumerate(list_ds_train):
        ...
        if ind_level is not None:
            try:
                ...
                ds_subset = ds[varname].isel(zIndex=ind_level) # zIndex', 'yIndex', 'xIndex'
        else:
...
def residual_zscore_var(conf, varname, ind_level=None, flag_float64=False, skipna=False):
    ...
    file_nums = int(conf['residual']['file_nums'])
    train_files = filenames[:file_nums]
    ...
    if ind_level is not None:
        try:
            ...
            ds_mean = ds_mean.isel(zIndex=ind_level)
            ds_std = ds_std.isel(zIndex=ind_level)
    ...
    for i_fn, ds in enumerate(list_ds_train):
        ...
        elif 'yIndex' in ds_out.coords and 'xIndex' in ds_out.coords:
            ds_out = ds_out.transpose("time_diff", 'yIndex', 'xIndex')
        else:
            raise ValueError("Expected coordinate names 'latitude/longitude' or 'lat/lon' not found in dataset.")
