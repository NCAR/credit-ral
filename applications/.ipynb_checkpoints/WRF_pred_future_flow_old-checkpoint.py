...
def apply_large_scale_correction(
    ...
    Returns (y_pred, match) where `match` is 0, 1, or None — same meaning
    as match_boundary_index. The caller can use `match` for logging.
    """
    ...
    # Pick the matching boundary slab directly — no interpolation.
    # Keeping the singleton time dim so the downstream shape matches what
...
def predict(rank, world_size, conf, p):
    ...
    units_upper_ERA5 = [1,]
    units_ERA5 = [1, 1, 1, 1 / float(np.sqrt(1000.0))]
