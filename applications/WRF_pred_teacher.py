...
def predict(rank, world_size, conf, p):
    ...
    N_teacher = conf['predict']['forecasts']['teacher_step']
    ...
    list_initial_ds = []
    for fn in filenames:
        list_initial_ds.append(filter_ds(get_forward_data(fn), varname_upper_air))
    initial_ds = xr.concat(list_initial_ds, dim='time')
    ...
    if filename_surface:
        ...
        for fn in filename_surface:
            list_surf_ds.append(filter_ds(get_forward_data(fn), varname_surface))
            
        surf_ds = xr.concat(list_surf_ds, dim='time')
        initial_ds = xr.merge([initial_ds, surf_ds])
    else:
        surf_ds = False
    ...
    # -------------------------------------------------------- #
    for i_init in range(N_steps):
        # ======================================================== #
        # DATA: summarize other relavent conf info
        # -------------------------------------------------------- #
        initial_ds_ = initial_ds.isel(time=slice(i_init, i_init+1))
        initial_time = initial_ds_['time'].values[0]
        # ---------------------------- #
        # has leap year
        # fcst_timesteps = np.arange(
        #     initial_time,
        #     initial_time + (N_teacher + 1) * np.timedelta64(lead_time_periods, "h"),
        #     np.timedelta64(lead_time_periods, "h"),
        # )
        # ---------------------------- #
        # no leap year
        step_hours = f'{lead_time_periods}h'
        end_time = initial_time + (N_teacher) * np.timedelta64(lead_time_periods, "h")
        ...
        # prediction section
        # -------------------------------------------------------- #
        for i_step in range(1, 1+N_teacher, 1):
            results = []
            ...
            # static field
            if filename_static is not None:
                initial_ds_ = xr.merge([initial_ds_, xarray_static.expand_dims(time=initial_ds_["time"].values,)])
            ...
            if filename_forcing is not None:
                ...
                # forcing field
                xarray_forcing["time"] = initial_ds_["time"]
                initial_ds_ = initial_ds_.merge(xarray_forcing)
            ...
            # rolling steps, use the previous output as input
                ...
                # a more general solution if forcing or dynmaic forcing are invloved
                    ...
                    if flag_static:
                        rolling_ds = xr.merge([rolling_ds, xarray_static.expand_dims(time=rolling_ds["time"].values,)])
