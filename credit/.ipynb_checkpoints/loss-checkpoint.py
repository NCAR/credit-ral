...
class MomentLoss(nn.Module):
    def __init__(
        ...
        """
        Loss = |mean_true - mean_pred| + |std_true - std_pred|
        computed over (H, W) for each (var, time), then reduced over batch and time.
        If multiple variables are selected, the losses are averaged/summed across them.
        """
        super().__init__()
    ...
    def forward(self, y_true: torch.Tensor, y_pred: torch.Tensor) -> torch.Tensor:
        # select multiple channels: result shape (B, C, T, H, W)
        y_pred_p = y_pred[:, self.var_inds, ...]
        y_true_p = y_true[:, self.var_inds, ...]
        
        # reshape: (B, C*T, H, W)
        B, C, T, H, W = y_pred_p.shape
        y_pred_p = y_pred_p.reshape(B, C * T, H, W)
        y_true_p = y_true_p.reshape(B, C * T, H, W)
        
        # compute spatial stats
        mean_pred = y_pred_p.mean(dim=(-2, -1))  # shape (B, C*T)
        mean_true = y_true_p.mean(dim=(-2, -1))
        mean_diff = (mean_true - mean_pred).abs()
        
        std_pred = y_pred_p.std(dim=(-2, -1), unbiased=self.std_unbiased)
        std_true = y_true_p.std(dim=(-2, -1), unbiased=self.std_unbiased)
        std_diff = (std_true - std_pred).abs()
        
        # final per-sample loss: shape (B, C*T)
        per_sample = mean_diff + std_diff
...
class WassersteinLoss(nn.Module):
    ...
    def forward(self, y_true: torch.Tensor, y_pred: torch.Tensor) -> torch.Tensor:
        ...
        # pick variables to compute Wasserstine loss
        y_pred_p = y_pred[:, self.var_inds, ...]
        y_true_p = y_true[:, self.var_inds, ...]
        
        # Flatten spatial dimensions
        B, C = y_pred_p.shape[:2]
        y_pred_flat = y_pred_p.reshape(B, C, -1)
        y_true_flat = y_true_p.reshape(B, C, -1)

        # Sort values along spatial dimension
        y_pred_sorted, _ = torch.sort(y_pred_flat, dim=-1)
        y_true_sorted, _ = torch.sort(y_true_flat, dim=-1)

        # Compute absolute difference (1D Wasserstein)
        wasserstein = torch.abs(y_pred_sorted - y_true_sorted)

        # Mean over spatial dimension
        loss = wasserstein.mean(dim=-1)  # shape: (B, C)
...
class VariableTotalLoss2D(torch.nn.Module):
    ...
    def __init__(self, conf, validation=False):
        ...
        # ------------------------------------------------------------- #
        # moment loss
        self.flag_moment_loss = conf['loss']['moment_loss']['activate']
        
        if self.flag_moment_loss:
            logger.info("Domain-wise moment loss applied")
            var_inds = conf['loss']['moment_loss']['var_inds']
            self.moment_lambda = conf['loss']['moment_loss']['lambda']
            self.moment_loss = MomentLoss(var_inds, reduction="mean")
            
        # ------------------------------------------------------------- #
        # moment loss
        self.flag_wasserstein_loss = conf['loss']['wasserstein_loss']['activate']
        
        if self.flag_wasserstein_loss:
            logger.info("Domain-wise wasserstein loss applied")
            var_inds = conf['loss']['wasserstein_loss']['var_inds']
            self.wasserstein_lambda = conf['loss']['wasserstein_loss']['lambda']
            self.wasserstein_loss = WassersteinLoss(var_inds, reduction="mean")
    ...
    def forward(self, target, pred):
        ...
        # Add moment loss
        if self.moment_loss:
            moment_loss_val = self.moment_loss(target, pred)
            loss += self.moment_lambda * moment_loss_val
            
        # Add Wasserstein loss
        if self.wasserstein_loss:
            wasserstein_loss_val = self.wasserstein_loss(target, pred)
            loss += self.wasserstein_lambda * wasserstein_loss_val
