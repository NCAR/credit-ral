...
class SKEBS(nn.Module):
    ...
    def __init__(self, post_conf):
        ...
        num_channels = self.channels * self.levels + post_conf["model"]["surface_channels"] + post_conf["model"]["output_only_channels"]
        if self.use_statics:
        ...
        if self.dissipation_type == "prescribed":
            ...
            self.backscatter_network = BackscatterUnet(num_channels, self.levels, self.nlat, self.nlon, architecture, padding)
        else:
    ...
    def initialize_pattern(self, y_pred):
        ...
        self.multivariateNormal = MultivariateNormal(torch.zeros(2, device=y_pred.device), torch.eye(2, device=y_pred.device))
    ...
    def forward(self, x):
        ...
        backscatter_pred = self.dissipation_scaling_coefficient * self.backscatter_filter * self.tropics_backscatter_filter * self.backscatter_network(x)

        if (
            (self.write_rollout_debug_files and not self.is_training)  # save out raw all backscatter prediction when not training
