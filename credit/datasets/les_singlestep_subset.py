...
class WRF_Dataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        ind_start = 0
        self.N_files = len(self.list_upper_ds)
        ...
        if "lead_time_unit" in param_outside:
            lead_time_unit = str(param_outside["lead_time_unit"])
        else:
            lead_time_unit = "h" # hourly dataset as default
        # ----------------------------------------------------------- #
        # collecting xr.datasets
        ...
        self.lead_time_unit = lead_time_unit
        
        # # -------------------------------------------------------------------------- #
        # # get sample indices from boundary upper-air files:
        # self.outside_file_year_range = [
        #     int(np.datetime_as_string(self.list_upper_ds_outside[0]["time"][0].values, unit="D")[-2:]),
        #     int(np.datetime_as_string(self.list_upper_ds_outside[-1]["time"][0].values, unit="D")[-2:]),
        # ]

        # XXXXXXXXXXXXXXXXXXXXXX
        # Extract time bounds per dataset (full datetime, not just day-of-month)
        self._outside_starts = np.array([_to_ns(ds["time"].values[0])  for ds in self.list_upper_ds_outside])
        self._outside_ends = np.array([_to_ns(ds["time"].values[-1]) for ds in self.list_upper_ds_outside])
        
        # Cache the pandas/cftime indexes to avoid repeated conversions
        self._outside_time_indexes = [ds.indexes["time"] for ds in self.list_upper_ds_outside]
        ...
        if "land_patch" in param_outside:
            self.flag_land_patch = param_outside["land_patch"]
        else:
            self.flag_land_patch = True
            
        self.total_len = 0
        for WRF_file_xarray in self.list_upper_ds:
            self.total_len += len(WRF_file_xarray["time"]) - self.total_seq_len + 1
    ...
    def _select_outside_window(self, time_round, history_len, lead_time_periods):
        
        times_req = [previous_second_steps(time_round, lead_time_periods, i) for i in range(history_len)]
        
        times_req = np.asarray(times_req, dtype="datetime64[ns]")
        ds_idx = np.searchsorted(self._outside_starts, times_req, side="right") - 1
        
        # Coverage check: time must be <= end of chosen dataset
        ends_for = self._outside_ends[ds_idx]
    
        # Group by dataset index to avoid per-time loops over xarray objects
        upper_parts = []
        surf_parts = []
        
        for k in np.unique(ds_idx):
            mask = (ds_idx == k); t_k = times_req[mask]
            
            t_k_pd = pd.to_datetime(t_k)
    
            time_index = self._outside_time_indexes[k]  # DatetimeIndex or CFTimeIndex
            ind_k = time_index.get_indexer(t_k_pd, method="backfill", tolerance=None)
            
            upper_parts.append(self.list_upper_ds_outside[k].isel(time=ind_k))
            surf_parts.append(self.list_surf_ds_outside[k].isel(time=ind_k))
            
        ds_upper = xr.concat(upper_parts, dim="time").sortby("time")
        ds_surf  = xr.concat(surf_parts,  dim="time").sortby("time")
        ds_outside = xr.merge([ds_upper, ds_surf], join="exact", compat="override")
        return ds_outside

    def _select_outside_at_time(self, time_round):
        t = np.asarray(time_round, dtype="datetime64[ns]")
        ind_year = np.searchsorted(self._outside_starts, t, side="right") - 1
        time_index = self._outside_time_indexes[ind_year]
        ts = pd.Timestamp(t)
        ind_date = time_index.get_indexer([ts], method="backfill", tolerance=None)[0]
        ds_upper_outside = self.list_upper_ds_outside[ind_year].isel(time=[ind_date])
        ds_surf_outside  = self.list_surf_ds_outside[ind_year].isel(time=[ind_date])
        return xr.merge([ds_upper_outside, ds_surf_outside], join="exact", compat="override")
    ...
    def __getitem__(self, index):
        ...
        ind_file = np.random.randint(0, high=self.N_files, dtype=int)
        ind_largest = len(self.list_upper_ds[int(ind_file)]["time"]) - (self.history_len + self.forecast_len + 1)
        ind_start_in_file = np.random.randint(0, high=ind_largest, dtype=int)
        ...
        if self.lead_time_periods_outside == 1 and self.history_len_outside == 1:
            ...
            time_round = next_n_second(time_boundary, self.lead_time_periods_outside, self.lead_time_unit)
            
        if self.history_len_outside == 1:
            ds_outside = self._select_outside_at_time(time_round)            
            # time_year = int(np.datetime_as_string(time_round, unit="D")[-2:])
            ...
            ds_outside = self._select_outside_window(
                time_round,
                history_len=self.history_len_outside,
                lead_time_periods=self.lead_time_periods_outside,
            )
            ...
            #     time_year = int(np.datetime_as_string(time_round_loop, unit="D")[-2:])
            #     ind_year = time_year - self.outside_file_year_range[0]
        ...
        if self.flag_land_patch:
            WRF_input, WRF_target, ds_outside = random_land_patch(WRF_input, WRF_target, ds_outside)
        else:
            WRF_input, WRF_target, ds_outside = random_patch(WRF_input, WRF_target, ds_outside)
        
        # pipe xarray datasets to the sampler
        sample = Sample_WRF(
            WRF_input=WRF_input.load(),
            WRF_target=WRF_target.load(),
            boundary_input=ds_outside.load(),
    ...
    def find_start_stop_indices(self, index):
        ...
        for init_time in self.init_time_list_np:
            for i_file, ds in enumerate(self.list_upper_ds):
                ...
                ds_year = int(np.datetime_as_string(time_value, unit="D")[-2:])
        ...
        for index in sampler:
            ...
            for k, _ in enumerate(self.init_time_list_np):
                ...
                if self.lead_time_periods_outside == 1 and self.history_len_outside == 1:
                    ...
                    time_round = next_n_second(time_boundary, self.lead_time_periods_outside)
