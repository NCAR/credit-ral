...
class WRF_Dataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        # ── OPT-4: pre-load static fields as numpy ──
        self.filename_static = filename_static
        if filename_static is not None:
            ds_static = get_forward_data(filename_static, use_chunks=False)
            ds_static = filter_ds(ds_static, varname_static).load()
            # Store as a dict of numpy arrays {varname: ndarray}
            self._static_np = {vname: ds_static[vname].values for vname in varname_static}
            self.varname_static = list(varname_static)
        else:
            self._static_np = None
            self.varname_static = []
        
        # Load it once; random_land_patch only needs the 2D array.
        landmask_var = ds_static["LANDMASK"]
        
        for dim in ("time", "level"):
            if dim in landmask_var.dims:
                landmask_var = landmask_var.isel({dim: 0})
        self._landmask = landmask_var.values.astype(np.float32)
        self._ny, self._nx = self._landmask.shape
        ...
        lead_time_periods_outside = param_outside["lead_time_periods"]
        # ----------------------------------------------------------- #
        # collecting xr.datasets
        ...
        self.lead_time_periods_outside = lead_time_periods_outside
        # -------------------------------------------------------------------------- #
    ...
    def __getitem__(self, index):
        ...
        WRF_input = WRF_subset.isel(time=slice(0, self.history_len, 1))
        ...
        if self.list_dyn_forcing_ds:
            ...
            dyn_forcing_subset = dyn_forcing_subset.isel(time=slice(0, self.history_len, 1))
        ...
        WRF_target = WRF_subset.isel(time=slice(self.history_len, ind_end_time, 1))
        ...
        if self.list_diag_ds:
            ...
            diagnostic_subset = diagnostic_subset.isel(time=slice(self.history_len, ind_end_time, 1))
        ...
        if self.lead_time_periods_outside == 1 and self.history_len_outside == 1:
            time_round = time_boundary
        else:
            time_round = next_n_hour(time_boundary, self.lead_time_periods_outside)
        ...
        if self.history_len_outside == 1:
            ...
            for i_time_backward in range(self.history_len_outside):
                time_round_loop = previous_hourly_steps(time_round, self.lead_time_periods_outside, i_time_backward)
        ...
        # spatial patch selection with numpy landmask ──
        ys, xs = random_land_patch(
            self._landmask, self._ny, self._nx, rng=self.rng
        )
        WRF_input = WRF_input.isel(south_north=ys, west_east=xs)
        WRF_target = WRF_target.isel(south_north=ys, west_east=xs)
        ds_outside = ds_outside.isel(south_north=ys, west_east=xs)
        
        WRF_input = WRF_input.load()
        WRF_target = WRF_target.load()
        ds_outside = ds_outside.load()
 
        # ── OPT-4 continued: inject static fields as xarray vars post-load ──
        if self._static_np is not None:
            n_input_times = len(WRF_input["time"])
            for vname, arr in self._static_np.items():
                # arr may be (ny, nx) or (1, ny, nx) — slice spatially
                spatial = arr
                # Remove leading singleton dims
                while spatial.ndim > 2 and spatial.shape[0] == 1:
                    spatial = spatial[0]
                # Apply the same spatial patch
                patched = spatial[ys, xs]  # (ph, pw)
                # Broadcast to (n_input_times, ph, pw) and assign
                tiled = np.broadcast_to(patched[np.newaxis, ...], (n_input_times,) + patched.shape)
                WRF_input[vname] = (("time", "south_north", "west_east"), tiled)
...
# class WRF_Dataset(torch.utils.data.Dataset):
#     """
#     Optimised WRF/regional-model Dataset.
 
#     Major differences from the original:
#     - Flat index lookup table instead of per-sample dict scan        (OPT-1)
#     - Cached length                                                   (OPT-2)
#     - Numpy landmask + integral image for O(1) patch search           (OPT-3)
#     - Static fields pre-loaded as numpy, not merged per sample        (OPT-4)
#     - Dask-backed lazy file access                                    (OPT-5)
#     - Direct variable slicing replaces xr.merge chains                (OPT-6)
#     - Numpy stacking for boundary data                                (OPT-7)
#     - Pre-sorted boundary time arrays for fast searchsorted           (OPT-8)
#     """
...
#         # ── OPT-5: lazy dask-backed open ──
#         all_ds = [get_forward_data(fn, use_chunks=True) for fn in filenames]
 
#         self.list_upper_ds = [filter_ds(ds, varname_upper_air) for ds in all_ds]
 
#         self.list_surf_ds = (
#             [filter_ds(ds, varname_surface) for ds in all_ds]
#             if filename_surface
#             else False
#         )
#         self.list_dyn_forcing_ds = (
#             [filter_ds(ds, varname_dyn_forcing) for ds in all_ds]
#             if filename_dyn_forcing
#             else False
#         )
#         self.list_diag_ds = (
#             [filter_ds(ds, varname_diagnostic) for ds in all_ds]
#             if filename_diagnostic
#             else False
#         )
 
#         # Store variable name lists for direct slicing later (OPT-6)
#         self.varname_upper_air = list(varname_upper_air)
#         self.varname_surface = list(varname_surface) if filename_surface else []
#         self.varname_dyn_forcing = list(varname_dyn_forcing) if filename_dyn_forcing else []
#         self.varname_diagnostic = list(varname_diagnostic) if filename_diagnostic else []
 
#         # ── OPT-1: flat index table ──
#         # Instead of scanning a dict with find_key_for_number every __getitem__,
#         # build two arrays:
#         #   _file_idx[i]   = which file global sample i belongs to
#         #   _local_idx[i]  = starting time index inside that file
#         # This turns the O(num_files) scan into O(1) array lookup.
#         file_indices = []
#         local_indices = []
#         for fid, ds in enumerate(self.list_upper_ds):
#             n_times = len(ds["time"])
#             n_samples = n_times - self.total_seq_len + 1
#             if n_samples <= 0:
#                 continue
#             file_indices.append(np.full(n_samples, fid, dtype=np.int32))
#             local_indices.append(np.arange(n_samples, dtype=np.int32))
 
#         self._file_idx = np.concatenate(file_indices)
#         self._local_idx = np.concatenate(local_indices)
#         # ── OPT-2: cached length ──
#         self._total_len = len(self._file_idx)
...
#             # Store as a dict of numpy arrays {varname: ndarray}
#             self._static_np = {
#                 vname: ds_static[vname].values for vname in varname_static
#             }
#             self.varname_static = list(varname_static)
#         else:
#             self._static_np = None
...
#         # ── OPT-3: pre-load landmask as numpy ──
#         # Load it once; random_land_patch only needs the 2D array.
#         landmask_var = ds_static["LANDMASK"]
...
#         # ================================================================== #
#         #  Boundary (outside) domain setup
#         # ================================================================== #
#         varname_upper_air_outside = param_outside["varname_upper_air"]
#         varname_surface_outside = param_outside["varname_surface"]
#         filenames_outside = sorted(param_outside["filenames"])
#         filename_surface_outside = param_outside["filename_surface"]
 
#         self.history_len_outside = param_outside["history_len"]
#         self.forecast_len_outside = param_outside["forecast_len"]
#         self.lead_time_periods_outside = param_outside["lead_time_periods"]
 
#         self.list_upper_ds_outside = []
#         self.list_surf_ds_outside = [] if filename_surface_outside else False
 
#         for fn_outside in filenames_outside:
#             ds_out = get_forward_data(fn_outside, use_chunks=True)
#             self.list_upper_ds_outside.append(filter_ds(ds_out, varname_upper_air_outside))
#             if filename_surface_outside:
#                 self.list_surf_ds_outside.append(filter_ds(ds_out, varname_surface_outside))
 
#         # ── OPT-8: pre-extract and sort boundary time arrays for fast lookup ──
#         self.outside_file_year_range = [
#             int(np.datetime_as_string(
...
#             )),
#         ]
#         # Store as plain numpy arrays — avoids repeated xarray coordinate access
#         self._outside_times = {
#             i: ds["time"].values for i, ds in enumerate(self.list_upper_ds_outside)
#         }
 
#     def __len__(self):
#         # OPT-2: return cached value
#         return self._total_len

#     def __getitem__(self, index):
#         # ── OPT-1: O(1) index lookup instead of dict scan ──
#         fid = int(self._file_idx[index])
#         ind_start_in_file = int(self._local_idx[index])

#         # ── Boundary handling: clamp to valid range ──
#         ind_largest = len(self.list_upper_ds[fid]["time"]) - (self.history_len + self.forecast_len + 1)
#         if ind_start_in_file > ind_largest:
#             ind_start_in_file = ind_largest

#         ind_end_in_file = ind_start_in_file + self.history_len + self.forecast_len
#         t_slice = slice(ind_start_in_file, ind_end_in_file + 1)
#         input_t_slice = slice(0, self.history_len)

#         # ── OPT-6: Direct variable slicing — no xr.merge chains ──
#         ds_upper = self.list_upper_ds[fid].isel(time=t_slice)

#         # Time metadata (cheap — just coordinate values)
#         all_times = ds_upper["time"].values
#         datetime_as_number = all_times.astype("datetime64[s]").astype(int)

#         # ── Build input dataset ──
#         WRF_input = ds_upper.isel(time=input_t_slice)
 
#         if self.list_surf_ds:
#             surf = self.list_surf_ds[fid].isel(time=t_slice).isel(time=input_t_slice)
#             WRF_input = xr.merge([WRF_input, surf])
 
#         if self.list_dyn_forcing_ds:
#             dyn = self.list_dyn_forcing_ds[fid].isel(time=t_slice).isel(time=input_t_slice)
#             WRF_input = xr.merge([WRF_input, dyn])
 
#         # ── OPT-4: Static fields — no expand_dims / assign_coords per sample ──
#         # The original expanded static fields on the time dimension and merged
#         # them into the xarray dataset every single sample. Instead, we inject
#         # them later at the numpy stage (after .load()) to avoid xarray overhead.
#         # If the downstream transform/sampler truly needs them as xarray vars,
#         # we do a lightweight merge here without expand_dims:
#         if self._static_np is not None:
#             # Broadcast static arrays to match input time dimension in numpy later
#             pass  # handled after spatial patch selection
 
#         # ── Build target dataset ──
#         ind_end_time = len(ds_upper["time"])
#         target_t_slice = slice(self.history_len, ind_end_time)
#         WRF_target = ds_upper.isel(time=target_t_slice)
 
#         if self.list_surf_ds:
#             surf_target = self.list_surf_ds[fid].isel(time=t_slice).isel(time=target_t_slice)
#             WRF_target = xr.merge([WRF_target, surf_target])
 
#         if self.list_diag_ds:
#             diag = self.list_diag_ds[fid].isel(time=t_slice).isel(time=target_t_slice)
#             WRF_target = xr.merge([WRF_target, diag])
 
#         # ── Boundary data ──
#         time_boundary = WRF_target["time"].values[0]
...
#         # ── OPT-7 + OPT-8: fast boundary lookup with pre-cached time arrays ──
#         if self.history_len_outside == 1:
#             # time_year = int(np.datetime_as_string(time_round, unit="Y"))
#             # ind_year = time_year - self.outside_file_year_range[0]
#             # ind_date = np.searchsorted(self._outside_times[ind_year], time_round)
#             # ds_outside = self.list_upper_ds_outside[ind_year].isel(time=slice(ind_date, ind_date + 1))
            
#             t_year = int(np.datetime_as_string(time_round, unit="Y"))
#             ind_date = np.searchsorted(self._outside_times[t_year], time_round)
#             file_idx = self._outside_idx_by_year[t_year]
#             ds_outside = self.list_upper_ds_outside[file_idx].isel(time=slice(ind_date, ind_date + 1))
            
#             if self.list_surf_ds_outside:
#                 surf_out = self.list_surf_ds_outside[ind_year].isel(time=slice(ind_date, ind_date + 1))
#                 ds_outside = xr.merge([ds_outside, surf_out])
#         else:
#             # ── OPT-7: collect slices, concat once ──
#             upper_slices = []
#             surf_slices = []
            
#             # for i_back in range(self.history_len_outside):
#             #     t_round_loop = previous_hourly_steps(
#             #         time_round, self.lead_time_periods_outside, i_back
#             #     )
#             #     t_year = int(np.datetime_as_string(t_round_loop, unit="Y"))
#             #     iy = t_year - self.outside_file_year_range[0]
#             #     idx = np.searchsorted(self._outside_times[iy], t_round_loop)
#             #     upper_slices.append(
#             #         self.list_upper_ds_outside[iy].isel(time=slice(idx, idx + 1))
#             #     )
#             #     if self.list_surf_ds_outside:
#             #         surf_slices.append(
#             #             self.list_surf_ds_outside[iy].isel(time=slice(idx, idx + 1))
#             #         )
...
#                 )
#                 t_year = int(np.datetime_as_string(t_round_loop, unit="Y"))
#                 idx = np.searchsorted(self._outside_times[t_year], t_round_loop)
#                 file_idx = self._outside_idx_by_year[t_year]
#                 upper_slices.append(
#                     self.list_upper_ds_outside[file_idx].isel(time=slice(idx, idx + 1))
#                 )
#                 if self.list_surf_ds_outside:
#                     surf_slices.append(
#                         self.list_surf_ds_outside[file_idx].isel(time=slice(idx, idx + 1))
#                     )
                    
#             # Reverse so latest time is last
#             ds_outside = xr.concat(upper_slices[::-1], dim="time")
#             if self.list_surf_ds_outside:
#                 ds_surf_out = xr.concat(surf_slices[::-1], dim="time")
#                 ds_outside = xr.merge([ds_outside, ds_surf_out])
...
#         # ── OPT-3: Spatial patch selection with numpy landmask ──
#         ys, xs = random_land_patch(
#             self._landmask, self._ny, self._nx, rng=self.rng
#         )
#         WRF_input = WRF_input.isel(south_north=ys, west_east=xs)
...
#         # ── OPT-10: Single .load() call per dataset ──
#         # .load() triggers the actual I/O. Do it once after all slicing is done
#         # so dask only reads the bytes we need.
#         WRF_input = WRF_input.load()
#         WRF_target = WRF_target.load()
#         ds_outside = ds_outside.load()
...
#                     patched[np.newaxis, ...], (n_input_times,) + patched.shape
#                 )
#                 WRF_input[vname] = (("time", "south_north", "west_east"), tiled)
...
class WRF_Predict(torch.utils.data.IterableDataset):
    def __init__(
        ...
        # ── OPT-8: pre-extract and sort boundary time arrays ──
        # Key by YEAR (matching the lookup logic) instead of sequential index
        self._outside_times = {}
        self._outside_idx_by_year = {}
        for i, ds in enumerate(self.list_upper_ds_outside):
            year = int(np.datetime_as_string(ds["time"][0].values, unit="Y"))
            self._outside_times[year] = ds["time"].values
            self._outside_idx_by_year[year] = i  # map year → file index
