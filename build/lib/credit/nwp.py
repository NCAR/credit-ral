...
def interpolate_to_model_level(regridded_nwp_data, output_grid, model_level_indices, variables):
    ...
    for var in upper_vars:
        ...
        interpolated_data[var] = {
            ...
            "data": np.array([np.interp(sampled_output_pressure[:, j, i], xp[:, j, i], fp_data[:, j, i]) for j in range(ny) for i in range(nx)]).reshape(ny, nx, len(model_level_indices)),
        }
    for var in vars_500:
        prs = 50000  # 500mb
        ...
        interpolated_data[var] = {
            ...
            "data": np.array([np.interp([prs], xp[:, j, i], fp_data[:, j, i]) for j in range(ny) for i in range(nx)]).reshape(ny, nx),
        }
